...
import { Sparkles, ShoppingBag, ArrowRight, RefreshCw, Search, Tag, Truck, ShieldCheck, Award, Mail, Check, SlidersHorizontal, Percent } from 'lucide-react';
...
const Home = ({ onAddToCart }) => {
  ...
  const filteredProducts = products.filter(p => {
    ...
    else if (priceRange === '1000to2000') matchesPrice = p.price >= 1000 && p.price <= 2000;
    else if (priceRange === 'over2000') matchesPrice = p.price > 2000;

    // Size Filter
    let matchesSize = true;
    if (sizeFilter !== 'All') {
      const sizesArr = Array.isArray(p.sizes) ? p.sizes : [];
      matchesSize = sizesArr.some(s => s.includes(sizeFilter));
    }

    // Sale Filter
    let matchesSale = true;
    if (saleOnly) {
      matchesSale = !!p.original_price;
    }

    return matchesCategory && matchesSearch && matchesPrice && matchesSize && matchesSale;
  });
      ...
      <GlassCard className="pulse-glow" style={styles.heroBanner}>
        <div style={styles.heroContent}>
          ...
          <p style={styles.heroDesc}>
            ยกระดับสไตล์ของคุณด้วยเสื้อผ้าสตรีทแฟชั่นทรงพรีเมียม ตัดเย็บจากผ้า Cotton 100% สวมใส่สบาย ดีไซน์ลิขสิทธิ์แท้เฉพาะแบรนด์ เสื้อเฮีย
          </p>
      ...
      <div style={styles.brandValueGrid}>
        ...
        <div style={styles.brandValueItem}>
          ...
          <div>
            ...
            <p style={styles.brandValueDesc}>สินค้าลิขสิทธิ์แท้จากแบรนด์ AURA</p>
          </div>
        </div>
        <div style={styles.brandValueItem}>
      ...
      <div id="catalog-section" style={styles.catalogSection}>
        <div style={styles.filterHeader}>
          <div style={styles.sectionTitleGroup}>
            ...
            <h2 style={styles.sectionTitle}>ALL PRODUCTS ({filteredProducts.length})</h2>
          </div>
        ...
        {showAdvancedFilters && (
          <GlassCard style={styles.advancedFilterCard}>
            <div style={styles.filterGroup}>
              <label style={styles.filterLabel}>ช่วงราคา (PRICE RANGE):</label>
              <div style={styles.filterPills}>
                {[
                  { id: 'All', label: 'ทุกราคา' },
                  { id: 'under1000', label: 'ต่ำกว่า 1,000 ฿' },
                  { id: '1000to2000', label: '1,000 - 2,000 ฿' },
                  { id: 'over2000', label: 'มากกว่า 2,000 ฿' },
                ].map(item => (
                  <button
                    key={item.id}
                    ...
                    style={{
                      ...
                      ...(priceRange === item.id ? styles.activeSubFilterPill : {})
                    }}
                  >
                    {item.label}
                  </button>
            ...
            <div style={styles.filterGroup}>
              <label style={styles.filterLabel}>ไซส์เสื้อผ้า (AVAILABLE SIZE):</label>
              <div style={styles.filterPills}>
                {['All', 'S', 'M', 'L', 'XL', 'XXL'].map(sz => (
                  <button
                    ...
                    style={{
                      ...styles.subFilterPill,
                      ...(sizeFilter === sz ? styles.activeSubFilterPill : {})
                    }}
                  >
                    {sz === 'All' ? 'ทุกไซส์' : sz}
                  </button>
            ...
            <div style={styles.filterGroup}>
              <label style={styles.filterLabel}>โปรโมชัน (OFFERS):</label>
              <button
                onClick={() => setSaleOnly(!saleOnly)}
                style={{
                  ...styles.subFilterPill,
                  ...(saleOnly ? styles.activeSubFilterPill : {})
                }}
              >
                <Percent size={12} style={{ marginRight: '4px' }} /> เฉพาะสินค้าลดราคา (ON SALE)
              </button>
            </div>
          </GlassCard>
        )}
        ...
        {loading ? (
          ...
          <div style={styles.noProducts}>
            <p>ไม่พบรายการสินค้าที่ตรงกับตัวกรองของคุณ</p>
          </div>
        ) : (
